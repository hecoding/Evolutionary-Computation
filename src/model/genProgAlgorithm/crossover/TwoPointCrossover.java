...
public class TwoPointCrossover implements CrossoverInterface {
	...
	private <T extends AbstractChromosome> void cross(ArrayList<T> population, T parent1, T parent2) {
		...
		int crossNode1 = random.nextInt(parent1Gene.getNodes() - 1) + 1; // all but the root
		int crossNode2 = random.nextInt(parent2Gene.getNodes() - 1) + 1; // all but the root
		
		Tree<Node> cross1 = parent1Gene.getNode(crossNode1);
		Tree<Node> cross2 = parent2Gene.getNode(crossNode2);
